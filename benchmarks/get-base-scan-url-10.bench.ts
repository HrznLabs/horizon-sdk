// Try to optimize getBaseScanUrl testing charCodeAt '0x' early
...
function getBaseScanUrlOpt(
  ...
  // By checking the 0x prefix before running the regex, we can avoid invoking the regex engine for non-hex inputs
  // And avoid evaluating full string if the prefix is wrong
  if (
    (len !== 42 && len !== 66) ||
...
const invalid1 = "1x1234567890abcdef1234567890abcdef12345678"; // wrong prefix

const start1 = performance.now();
for (let i = 0; i < 1000000; i++) {
  getBaseScanUrlOrig(addr1);
  getBaseScanUrlOrig(tx1);
  try { getBaseScanUrlOrig(invalid1); } catch (e) {}
}
...
for (let i = 0; i < 1000000; i++) {
  ...
  try { getBaseScanUrlOpt(invalid1); } catch (e) {}
}
