...
function formatUSDCOpt(
  ...
  if (compact) {
    // Optimization: replace computed bigints with literals.
    // 1000n * 1000000n = 1000000000n
    let divisor = 1n;
    let unit = '';

    // Notice we skip 1000n * USDC_MULTIPLIER_BIGINT and just hardcode it
    if (absAmount >= 1000000000000000000n) {
      divisor = 1000000000000000000n;
      unit = 'T';
    } else if (absAmount >= 1000000000000000n) {
      divisor = 1000000000000000n;
      unit = 'B';
    } else if (absAmount >= 1000000000000n) {
      divisor = 1000000000000n;
      unit = 'M';
    } else if (absAmount >= 1000000000n) {
      divisor = 1000000000n;
      unit = 'K';
    }
