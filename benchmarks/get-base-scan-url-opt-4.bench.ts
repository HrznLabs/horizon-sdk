...
function getBaseScanUrlHoisted(hashOrAddress: string, type?: 'address' | 'tx', testnet: boolean = true) {
...
function getBaseScanUrlOpt2(hashOrAddress: string, type?: 'address' | 'tx', testnet: boolean = true) {
  const len = hashOrAddress.length;
  let isAddress = false;
  let isTx = false;

  if (len === 42) {
    isAddress = ADDR_REGEX.test(hashOrAddress);
  } else if (len === 66) {
    isTx = TX_REGEX.test(hashOrAddress);
  }
...
for (let i = 0; i < ITERATIONS; i++) {
  getBaseScanUrlHoisted(address);
  getBaseScanUrlHoisted(tx);
}
...
for (let i = 0; i < ITERATIONS; i++) {
  getBaseScanUrlOpt2(address);
  getBaseScanUrlOpt2(tx);
}
end = performance.now();
console.log(`Optimized length check + regex time: ${(end - start).toFixed(2)}ms`);
