...
## 2026-06-25 - Unbounded String DoS in Formatting Options
**Vulnerability:** The formatting utility functions `formatUSDC`, `formatBps`, and `formatAddress` safely bounded their main string inputs, but they accepted unbounded strings for their configuration options (`prefix`, `suffix`, `separator`). This allowed memory exhaustion DoS vectors via these option parameters when they were concatenated during formatting.
**Learning:** Security validations for unbounded operations (like memory-heavy string concatenation) must be applied to all variable-length inputs, not just the primary argument. Configuration options objects are often overlooked as potential DoS vectors.
**Prevention:** Enforce strict length limits (e.g., `if (options?.prefix && options.prefix.length > 256)`) on all optional string parameters at the very beginning of utility functions before they are used in string evaluation or concatenation.
