...
## 2026-07-03 - String Coercion DoS in Object Properties
**Vulnerability:** While `String()` coercion in `formatUSDC`, `formatBps`, and `formatAddress` was introduced to prevent length property spoofing, it inadvertently created a new Denial of Service (DoS) vector. When a malicious object with an overridden `toString()` method returning an unbounded string is passed, the `String()` coercion explicitly executes the `toString()` method before any length checks occur, resulting in memory exhaustion.
**Learning:** Coercing untrusted inputs to strings (e.g., using `String()`) is unsafe because it executes arbitrary `toString()` methods on objects, which can be computationally expensive or memory intensive.
**Prevention:** Always use strict runtime type checking (`typeof options.property !== 'string'`) to validate inputs before assigning or processing them, instead of relying on implicit or explicit type coercion.
