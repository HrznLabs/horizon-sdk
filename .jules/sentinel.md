...
## 2026-06-25 - Unbounded String Operations in UI Formatting
...
## 2025-02-02 - Prototype Pollution in getNetwork Lookups
**Vulnerability:** Direct dictionary lookup `NETWORKS[chainId]` using untyped/unvalidated string parameters allowed access to Object prototype properties (e.g., `__proto__`, `constructor`), potentially leading to unexpected behavior or DoS.
**Learning:** Type assertion `as keyof typeof NETWORKS` only guarantees type safety at compile time. At runtime in Web3 environments, users or APIs often supply chain IDs as strings (even when typed as `number`), bypassing TS checks and hitting the dictionary directly.
**Prevention:** Always validate existence of dictionary keys dynamically constructed from external inputs using `Object.prototype.hasOwnProperty.call(dict, key)`. Additionally, type such parameters as `number | string` to reflect actual runtime Web3 usage and prevent upstream coercion bugs while strictly ensuring the key exists.
