...
## 2024-05-27 - Short-circuit Zero in Formatting Functions
**Learning:** In complex formatting functions (e.g., `formatUSDC`), formatting a `0n` amount incurs surprisingly high performance costs due to unnecessary BigInt divisions, max/min decimal comparisons, array modifications, and substring logic.
**Action:** Always add an explicit short-circuit for zero amounts (e.g., `if (amount === 0n)`) at the beginning of complex formatting functions to bypass unnecessary operations and drastically improve performance for this common edge case.
