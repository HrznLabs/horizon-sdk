...
## 2024-12-05 - [Optimize toBytes32 String Padding and Regex Compilation]
**Learning:** In `toBytes32`, dynamically evaluating `/^[0-9a-fA-F]*$/` and performing string slicing (`str.slice(2)`) incurs garbage collection overhead and regex compilation time on every call. Replacing dynamic checks with a hoisted `HEX_VALIDATION_REGEX` and checking string length to pad the 0x-prefixed string directly avoids intermediate allocations and string concatenations.
**Action:** When working with 0x-prefixed hexadecimal strings, avoid slicing off the prefix for validation and length calculations if possible. Instead, hoist regular expressions to module scope and calculate target lengths based on the fully prefixed string (e.g., target length 66 instead of 64). Padding the whole string directly avoids memory re-allocation.
