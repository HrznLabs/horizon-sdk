...
## 2023-10-27 - Optimizing BigInt numeric formatting
**Learning:** For string formatting utilities dealing with native `BigInt` parameters (like USDC token values), relying on string manipulation methods (like `.toString().padStart().substring()`) and loops creates significant object allocation and Garbage Collection overhead. Pure arithmetic operations (like `(remainder * 100n) / divisor` to extract two decimal places) run ~20% faster than formatting to a padded string and clipping it. Furthermore, accessing characters for zero trimming loops via `.charCodeAt(i) === 48` avoids allocating single-character strings compared to array indexing `[i] === '0'`.
**Action:** Always favor native math operations (modulo, division) for extracting portions of BigInts before resorting to string allocations, and use `.charCodeAt` for tight string character comparison loops.
