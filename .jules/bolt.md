...
## 2024-12-13 - [Optimize Hex Validation Bounds Check]
**Learning:** In fixed-pattern string validations (like checking if characters are valid hex in `toBytes32`), replacing multiple mathematical bounds checks (`!(code >= 48 && code <= 57) && !(code >= 65 && code <= 70) && !(code >= 97 && code <= 102)`) with a bitwise equivalent (`(code ^ 48) > 9 && (((code | 32) - 97) >>> 0) > 5`) is significantly faster (~15-25% in V8). The unsigned right shift `>>> 0` correctly handles character codes `< 97` by avoiding negative results that would break the comparison.
**Action:** When validating fixed sets of ASCII characters (like hex or numbers) in frequent hot paths, prefer bitwise bounds checking over multiple mathematical comparisons.
