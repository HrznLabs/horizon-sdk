...
## 2024-05-23 - Buffer Hoisting in Hot Path
**Learning:** Hoisting the `Uint8Array` buffer in `randomBytes32` to module scope (to avoid re-allocation) yielded an ~24% speed improvement (476ms to 360ms for 100k iterations).
**Action:** When a utility function allocates a fixed-size buffer on every call, consider hoisting it to module scope if thread-safety/concurrency allows (JS is single-threaded so it's usually safe for sync functions).
