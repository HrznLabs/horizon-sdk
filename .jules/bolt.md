...
## 2024-12-04 - [Optimize formatUSDC Compact Calculation]
**Learning:** In `formatUSDC` compact formatting, using mathematical extraction to extract fractional values (e.g. `remainder * 100n / divisor`) and building string representations dynamically using `+` and character index access (`str[0]`) reduces execution overhead by ~40% compared to using `.padStart()` with a `while` loop over strings for trailing zero trimming. V8 heavily optimizes pure numeric math and basic string concatenations when building short decimal combinations.
**Action:** Whenever a specific decimal-level formatting or truncation is needed without `Intl.NumberFormat`, prioritize integer calculation logic to construct the desired decimal places directly. Only fall back to `.padStart()` and string loops if dynamic length requires it.
