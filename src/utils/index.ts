...
export function parseUSDC(amount: string | number): bigint {
  ...
  if (len === 0) throw new Error('Invalid USDC amount format: Empty input.');
  ...
  for (let i = start; i < len; i++) {
    ...
    if (code === 46) { // '.'
      ...
      throw new Error('Invalid USDC amount format: Commas are not allowed, please remove thousands separators.');
    } else if (code === 36) { // '$'
  ...
  if (integerPartStr === "" && fractionPartStr === "") {
    throw new Error('Invalid USDC amount format: Empty input.');
  }
