...
export function parseUSDC(amount: string | number): bigint {
  ...
  // Validate chars and accumulate value mathematically in a single pass
  for (let i = start; i < len; i++) {
    ...
    if (code >= 48 && code <= 57) {
      ...
      if (hasDot) throw new Error('Invalid USDC amount format: Multiple decimal points found.');
      hasDot = true;
    } else if (code === 44) { // ','
  ...
  // Security: Ensure strings with only formatting characters (like "." or "-.") don't evaluate to 0
  if (!hasDigits) {
    throw new Error('Invalid USDC amount format');
  }

  // Scale fraction based on length
  const power = POWERS_OF_10[USDC_DECIMALS - fracLen];
  const val = intPartNum * USDC_MULTIPLIER_BIGINT + fracPartNum * power;
