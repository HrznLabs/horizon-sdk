...
export function formatUSDC(
  ...
  if (options?.prefix && options.prefix.length > 256) {
    throw new Error('Prefix too long: exceeds maximum input length');
  }
  if (options?.suffix && options.suffix.length > 256) {
    throw new Error('Suffix too long: exceeds maximum input length');
  }
...
export function formatAddress(
  ...
  if (options) {
    if (options.separator && options.separator.length > 256) {
      throw new Error('Separator too long: exceeds maximum input length');
    }
