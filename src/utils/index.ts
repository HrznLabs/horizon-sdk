...
export function parseUSDC(amount: string | number): bigint {
  ...
  let intPartNum = 0n;
  let fractionPartNum = 0n;
  let fracLen = 0;
  let isFraction = false;
  let hasDigits = false;

  // Validate chars, find dot, and build number mathematically
  for (let i = start; i < len; i++) {
    ...
    if (code >= 48 && code <= 57) {
      ...
      if (isFraction) {
        ...
        fractionPartNum = fractionPartNum * 10n + digit;
      } else {
  ...
  let val = intPartNum * USDC_MULTIPLIER_BIGINT;
  if (fracLen > 0) {
    // Use pre-calculated power to scale fraction correctly
    val += fractionPartNum * POWERS_OF_10[USDC_DECIMALS - fracLen];
  }
