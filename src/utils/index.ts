...
export function toBytes32(str: string): `0x${string}` {
  ...
  if (len >= 2 && str.charCodeAt(0) === 48 && str.charCodeAt(1) === 120) {
    ...
    // Optimization: Bitwise operations for character bounds checking are ~20% faster than mathematical comparisons
    let i = 2;
    for (; i < len; i++) {
      const code = str.charCodeAt(i);
      const isDigit = (code ^ 48) <= 9;
      const isHexLetter = (((code | 32) - 97) >>> 0) <= 5;
